...
import { Bell, Menu, Check, WifiOff, LogOut, User } from 'lucide-react';
...
const CandidateNavbar = ({ onToggleSidebar }) => {
  ...
  const userEmail = currentUser?.email || 'candidate@evolvevita.com';
  ...
  const getPageTitle = () => {
    ...
    if (path.includes('/dashboard')) return 'Candidate Dashboard';
    if (path.includes('/profile')) return 'My Developer Profile';
    if (path.includes('/resume')) return 'Resume Intelligence & Analysis';
    if (path.includes('/jobs')) return 'Target Job Matches';
    if (path.includes('/skill-gap')) return 'Skill Gap Analysis';
    if (path.includes('/learning')) return 'Upskilling & Learning Growth';
    if (path.includes('/settings')) return 'Account Configurations';
    ...
    <header className="navbar">
      ...
      <div className="navbar-right">
        ...
        <div 
          ...
          title={isBackendOnline ? "Backend services online" : "Backend services unavailable (http://127.0.0.1:8001)"}
        >
          {isBackendOnline ? (
            <>
                  ...
                  notifications.map(n => (
                    <div 
                      ...
                      style={{ 
                        ...
                        borderLeft: n.read ? 'none' : '3px solid var(--accent-secondary)',
                        display: 'flex',
                        flexDirection: 'column',
                        gap: '0.2rem'
                      }}
        ...
        <div style={{ position: 'relative' }}>
          ...
          {showUserMenu && (
            ...
            >
              ...
              <button 
                onClick={() => { setShowUserMenu(false); navigate('/candidate/settings'); }}
                className="btn-ghost"
